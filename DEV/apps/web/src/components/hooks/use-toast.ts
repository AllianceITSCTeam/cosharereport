...
type IAction =
  | { type: IActionType['ADD_TOAST']; toast: IToasterToast }
  | { type: IActionType['UPDATE_TOAST']; toast: Partial<IToasterToast> }
  | { type: IActionType['DISMISS_TOAST']; toastId?: IToasterToast['id'] }
  | { type: IActionType['REMOVE_TOAST']; toastId?: IToasterToast['id'] };
...
function toast({ ...props }: IToast) {
  const id = genId();
  const update = (p: IToasterToast) => dispatch({ type: 'UPDATE_TOAST', toast: { ...p, id } });
